...
[![npm version](https://img.shields.io/npm/v/chromafix?color=0072b2)](https://www.npmjs.com/package/chromafix)
[![minzipped size](https://img.shields.io/bundlephobia/minzip/chromafix?color=009e73)](https://bundlephobia.com/package/chromafix)
[![zero dependencies](https://img.shields.io/badge/dependencies-0-e69f00)](./package.json)
...
[![license](https://img.shields.io/npm/l/chromafix?color=555)](./LICENSE)
...
that reads those variables recolors instantly. Turning it off removes them.
...
```js
import { createChromafix } from "chromafix";
...
```tsx
import { ColorblindWidget } from "chromafix/react";
...
wins over stylesheet rules, so there are no specificity games and no `!important`.
...
| ------------- | ------------------------------------------------------------- | -------------------------- | -------------------------------------------------------- |
| `tokens`      | `Record<string, Role>`                                        | `{}`                       | Your CSS variables → palette roles. Required to recolor. |
...
| `onChange`    | `(type) => void`                                              | —                          | Called whenever the selection changes.                   |
...
`{ name, hint }` object that adds a dimmed subtitle — ideal for pairing a
...
```js
import { applyPalette, PALETTES, CVD_TYPES, ROLES } from "chromafix";
...
- Options are a native `<fieldset>` radio group — arrow-key navigation for free.
...
import time — importing or calling into chromafix in a Node/SSR context is a safe
