...
- Initial release.
- Framework-agnostic core (`chromafix`): `createChromafix`, `applyPalette`,
...
- Colorblind-safe **palette switching**: each of the seven deficiency types
  ...
  remapping the site's CSS variables inline on `:root` — no pixel filter.
...
- React adapter (`chromafix/react`): SSR-safe `<ColorblindWidget />`.
